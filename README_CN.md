...
**环境要求：** macOS 14+ · 已安装 [Kimi Code](https://github.com/MoonshotAI/kimi-code) CLI（`npm install -g @moonshot-ai/kimi-code`）
...
启动 URL：命令行参数 → `$KIMINI_URL` → 自动发现
（`~/.kimi-code/server/lock` + `server.token`，必要时执行 `kimi server run` 拉起）。
