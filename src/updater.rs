        ...
        fn resolves_framework_next_to_a_packaged_executable() {
            assert_eq!(
                bundled_framework_binary(Path::new(
                    "/Applications/Kimini.app/Contents/MacOS/kimini"
                )),
                Some(PathBuf::from(
...
#[cfg(target_os = "macos")]
pub use platform::Updater;

/// Non-macOS packages currently delegate updates to their package channel.
...
impl Updater {
    ...
    /// Returns false so the caller opens the latest release page.
    pub fn check_now(&self) -> bool {
        false
    }
}
